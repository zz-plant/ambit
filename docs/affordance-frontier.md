...
- **Scope is checked, not enforced.** `ambit authority scope <target>` verifies whether a grant's scope covers a target — an authority row carrying `repo:owner/name` is now answerable, and a grant scoped elsewhere is named as excluded. The check is a report, not a gate.
