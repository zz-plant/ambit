...
> **Status: a design document, not a plan of record.** This is a long statement of intent with no dates, no owners and nothing that can close. It reads as a roadmap and functions as an essay; anything here that is going to be built belongs in an issue, where it can be assigned and closed.

Ambit today is a capability graph. It reads your configuration, places it on a curated tech tree, and answers questions about the structure: what is reached, what the next step is, what would break if a given thing disappeared.
...
| **2 · Assurance** | distinguish *configured* from *demonstrated* — `installed ≠ callable ≠ working ≠ reliable ≠ authorized ≠ appropriate` |
...
What each section still lacks is collected here and stated nowhere else, so a reader deciding whether the project is real does not have to hunt through every section for it.
...
| [2](#2-put-the-human-and-the-machines-in-the-graph--partly-built) | Put the human and the machines in the graph | partly built | Cost and risk tolerance are not modeled: a preference is a word matched to an alternative's properties, not a budget or a threshold, so a plan cannot say *this step is not worth your attention* or *you asked not to spend more than this*. The infrastructure manifest is read but not probed by the engine (the server probes it for the visualizer), so a device's reachability gates nothing. The server now also reads the local Docker socket, so containers appear as live services; Tailscale nodes and GPU VRAM capacity are still authored, not probed. |
...
| [4](#4-detection-becomes-verification--built-and-gates) | Detection becomes verification | built, and gates | Gates on evidence: a failing check demotes, a threshold a person set promotes (§12.6), and real use counts (§13.2). The gate is consulted at every decision surface and enforced in two places, the apply path (§10) and the control plane (§15); everywhere else it is read and not obeyed, and a check whose failure flipped a capability to `broken` is re-verified by hand. |
| [5](#5-goal--capability-delta--partly-built) | Goal → capability delta | partly built | A goal naming no known vocabulary has no route in: *maintain the homelab unattended* reaches three known capabilities and does not synthesize *service health observation* or *bounded restart authority* out of whole cloth. Whole-strategy alternatives (broad authority, a capability-scoped MCP, a kubernetes migration) have to be authored before a goal can be offered as a choice at that grain. `ambit goal --judge` routes an unmatched goal through a typed decision model on this machine as a Choice over the tree, and prints the likeliest node with its probability; it suggests and writes nothing, and the MCP `ambit_goal` does not offer it yet. |
...
| [7](#7-the-ledger--shipped) | The ledger | shipped | Use is recorded and counts toward promotion, and it carries no object, so which capability was exercised *on what* is unanswered (§13.9). |
| [7b](#7b-affordance-domains--built-derived) | Affordance domains | built, derived | Derived from structure, not from keywords. The map draws `physical`, which is a node domain. The derived domains have no column, but the map marks a node a person approves or supplies, or one that runs on a device, and the detail panel names who. The era grammar is deliberately designed, and adding columns for what `ambit graph affordances` computes would be design work, not model work. The theory's *environment* term, an affordance holding in one workspace and not another, has no representation. |
| [8](#8-a-runtime-adapter-layer--partly-built) | A runtime adapter layer | partly built | A runtime-owned capability surface exists as an export and a reader; no runtime publishes one yet, so file-parsing is still the path that runs. Runtime adapters for emerging assistants (Cline, Roo Code, Continue, Zed) and an IDE sidebar extension embedding the AG-UI stream are unbuilt. |
| [9](#9-authority-as-a-first-class-edge--built-and-mediates-one-thing) | Authority as a first-class edge | built, and mediates one thing | The one place is a grant narrowing when what it rests on fails. Nothing forces a *runtime* to consult the gate. |
| [9b](#9b-the-record-it-writes-std-07--shipped) | The record it writes: STD-07 | shipped | Nothing reads the records. Ambit is the second publisher of the shape and there is still no consumer. |
| [10](#10-a-second-generation-mcp--partly-built) | A second-generation MCP | partly built | A capability with no declared check applies unverified and reports that it did; refusing outright would break the acquisition path for every capability whose recipe is not a check, so it stays a report until §3's multi-step verification can back it. Nothing that changes the world is exposed over MCP: `ambit_simulate` and `ambit_propose` exist, and there is no `apply_step`, `request_approval` or `rollback`. Stdio is the only transport; HTTP/SSE and WebSocket transports, sandboxed container/WASM execution, and an authenticated CLI installer (`ambit install`) are unbuilt. |
| [11](#11-the-visualizer-becomes-a-negotiating-surface--built-and-shipping) | The visualizer becomes a negotiating surface | built and shipping | Only the state and run subset of AG-UI; no tool-call or reasoning events, by design. Canvas ergonomics for large graphs (light theme tokens, a minimap HUD, high-resolution SVG/PNG exports, and historical timeline playback) are unbuilt. |
| [12](#12-the-long-running-agent--built-except-the-install) | The long-running agent | built, except the install | Homebrew installs it (§12.9); the npm package is built and unpublished, so `npx` is not yet a path in. Out-of-band dispatch (§12.10) pushes a proposal to Slack, Discord, Telegram or ntfy; the reply still comes back through `ambit approve` on a machine holding the key, by design. |
| [13](#13-expanding-the-ambit--built) | Expanding the ambit | built | Authority and evidence refer to objects, and the era tree does not: *read repo A* and *read repo B* are two recorded facts and still one node. Tree overlays (`.ambit/techtree.json`) and time-bounded grants (`authority grant --ttl`) are built; an overlay node is seeded like a curated one, and an expired grant decides nothing without being rewritten, so what stood before the elevation stands after it. |
| [14](#14-the-economic-half--partly-built) | The economic half | partly built | The marketplace, deliberately, until observed demand fills the catalog; and the reports need weeks of recorded work before they are worth trusting. Granular token/dollar attribution per capability and local vs hosted provider arbitrage are unbuilt. |
| [15](#15-the-control-plane--built-against-a-simulated-environment) | The control plane | built, against a simulated environment | One real `EnvironmentAdapter` implementation, and a runtime that routes through the proxy by default. It has its own entry point and no `ambit` verb, so nothing reaches it unless a caller chooses to. Pre-execution dry-run blast radius simulation and emergency break-glass logging are unbuilt. |
| [16](#16-federation-fleet-governance-and-team-agency--partly-built) | Federation, fleet governance, and team agency | partly built | Multi-tenant graph synchronization, GitOps PR-driven governance, enterprise role-based access control, and compliance audit exports (SOC2) are unbuilt. |
...
Typing the edges was also a correctness fix, not tidiness. `providersOf` matched three English sentences, so an adapter phrasing one differently dropped a provider out of both analyses with nothing failing: a capability with two providers still reported as a single point of failure.
...
Built: eleven capabilities carry `acquisition.alternatives`, and `ambit goal` attaches them to each step. Alternatives, not one blessed answer, because the trade-off is rarely setup time: the hosted embedding API is three minutes against ten and costs money and a data boundary, and the plan says so.
...
Inference is built, from the signals and not from the prose: §12.2. A runtime already states that a command was not found, that permission was denied, that a host was unreachable, that an MCP call returned an error kind — and the telemetry bridge hands those over for classification, so the ledger fills from work instead of from someone remembering to record it. A failure whose shape says nothing is left unclassified, never guessed at, and one that cannot be attributed to a capability is kept anyway, since "this keeps failing and the model cannot name it" is a finding about the model.
...
Built. `frontier_snapshots` records every capability's state on each seed, written only when the state differs from the previous observation, so the table logs changes and not runs. `ambit history` lists the observations, and `ambit history since [when]` compares two.
...
Classification compares against the ids recorded in the snapshot, not timestamps. `created_at > taken_at` looked equivalent and was not: `datetime('now')` resolves to the second, so two seeds inside the same second classified every addition as pre-existing.

`ambit history` also distinguishes a fourth thing: **vocabulary**. A node the past observation never saw, everything supplying which the past observation did see, is Ambit having started to model a part of the system, not the system having changed: the release that introduced action nodes, or a capability added to the curated tree that your existing tools already provide. Those are described and not counted, so `frontier_now` stays on the same basis as `frontier_then`. Without it, upgrading would have reported a dozen capabilities gained on a machine where nothing happened, which is exactly what the ledger exists not to say.

Use is written now — the telemetry bridge records every capability a run exercised, and a successful use counts as evidence (§13.2) — but it carries no object, so *which repository* was committed to is not a question the ledger can answer yet (§13.9). It also records demonstrated reliability beside reach: each snapshot carries a `verified` count and an id→lifecycle map, so `ambit history` reports a capability that stopped working as `diminished` (with `reason: verification failing`) while `frontier_now` stays flat — the check started failing, nothing was removed.
...
- The editor is an assistant runtime too. Cline, Roo Code, Continue.dev, and Zed each maintain assistant sessions and tool registries. Extending the adapter layer to read their configurations keeps the graph unified across standalone agents and editor sidecars. A native IDE sidebar extension (VS Code, Cursor) embedding the AG-UI event stream eliminates switching to the browser, while respecting the loopback boundary: the extension reads the graph over HTTP and requests approvals through the authenticated CLI.
...
Conformance is declared as level 2 for those four kinds and nothing else. `action` and `outcome` are deliberately not emitted: the environment adapter is simulated, so an action record from here would attest to a fixture. The manifest in `server.json` says so, because the standard's own adoption note is that declaring a level honestly is conformance and claiming one the log does not earn is not.

A consumer is the thing worth building next, because a record format with publishers and no readers is a documentation format.
...
`cg_simulate_acquisition` matters as much as `cg_plan`: showing the graph as it *would* be, before anything changes, is what makes approval meaningful and not ceremonial.
...
3. **Three lenses**, switched on the map itself: Standard; Attention (nodes shaded by how often a person had to step in), offered once the ledger has recorded any; and Authority (each reached node by what it may do without asking, with "no grant yet" apart from a refusal). The host-cluster lens was sunset with the 3D views. The shared-credentials lens was withdrawn when it turned out to colour nodes by a substring of their id; credentials are the engine's report, `ambit credentials`.
...
What the visualizer still lacks is physical ergonomics for large setups. When a graph scales past several hundred nodes, navigation requires a minimap HUD. High-glare environments demand a light theme option using vanilla CSS tokens. Teams documenting capability architectures require high-resolution canvas export (SVG, PNG, standalone HTML). And scrubbing backwards across historical snapshots would turn the ledger into an interactive replay, letting an operator watch the frontier expand over time.
...
Ten parts: nine built and tested in `src/engine/agent-loop.test.ts`, `src/mcp/server.test.ts` and `src/engine/dispatch.test.ts`, one waiting on a token. The reference is [the deep dive](./deep-dive.md). What each one decided, and why:
...
3. **One question before acting.** `ambit_can` answers `yes`, `ask` or `no` in one round trip from three indexed reads, and a `no` files the deficit in the same call, so the habit costs one call and not two.
...
9. **One line to install — half built.** `brew install zz-plant/tap/ambit` on macOS or Linux is one line, and the release workflow keeps the tap current. `claude mcp add ambit -- npx -y ambit-cli mcp` on a machine with Node and nothing else is not: the package is built and the publish workflow waits on a token. Until it lands, an agent on a machine without Homebrew cannot act on the decision that Ambit would help without a checkout.
...
Eleven changes, built, with acceptance tests in `src/engine/expansion.test.ts`, `src/engine/ttl-authority.test.ts` and `src/engine/techtree-overlay.test.ts`:
...
7. **Learning from the refusals.** Approval was always recorded and refusal was not, so the graph could learn what a yes looked like and never a no. `ambit reject` records the other half; a trait needs three decisions before it counts and one that went both ways reads as contested, not settled by majority.
...
The gate §9 describes stays a report until something consults it before acting. The interceptor is that something: it sits in front of the tool call, and a call whose prerequisites are missing, whose foundation is failing, or whose caller is not permitted is refused with nothing on the machine changed.
...
Production enforcement requires intercepting live tool calls across real transports. Before prompting the operator for approval, the interceptor can run `ambit_simulate` to show the expected blast radius directly in the decision prompt. During active incidents, an audited emergency override (`--break-glass`) allows privileged execution while logging immutable, hash-chained records.
...
Four capabilities complete the team layer:
...
4. **Compliance and SIEM audit exports.** The ledger already produces hash-chained records. Formatting exports for SOC2 and ISO27001 compliance, and streaming audit events to corporate SIEM systems (Splunk, Elastic), turns agent oversight into verifiable enterprise infrastructure.
