...
| [8](#8-a-runtime-adapter-layer--partly-built) | A runtime adapter layer | partly built | A runtime-owned capability surface exists as an export and a reader; no runtime publishes one yet, so file-parsing is still the path that runs. |
...
| [12](#12-the-long-running-agent--built-except-the-install) | The long-running agent | built, except the install | The package is not on the registry (§12.9), so every path in still starts with a checkout. |
| [13](#13-expanding-the-ambit--built) | Expanding the ambit | built | Authority and evidence refer to objects, and the era tree does not: *read repo A* and *read repo B* are two recorded facts and still one node. |
| [14](#14-the-economic-half--partly-built) | The economic half | partly built | The marketplace, deliberately, until observed demand fills the catalog; and the reports need weeks of recorded work before they are worth trusting. |
