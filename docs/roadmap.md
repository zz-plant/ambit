...
| [7b](#7b-affordance-domains--built-derived) | Affordance domains | built, derived | Derived from structure, not from keywords. The map draws `physical`, which is a node domain; the derived domains are not, so the map has no column for them. The era grammar is deliberately designed, and adding columns for what `ambit graph affordances` computes would be design work, not model work. The theory's *environment* term, an affordance holding in one workspace and not another, has no representation. |
