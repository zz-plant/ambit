...
| [12](#12-the-long-running-agent--built-except-the-install) | The long-running agent | built, except the install | The package is not on the registry or Homebrew (§12.9), so every path in still starts with a checkout. Out-of-band dispatch (§12.10) pushes a proposal to Slack, Discord, Telegram or ntfy; the reply still comes back through `ambit approve` on a machine holding the key, by design. |
...
9. **One line to install — unbuilt.** `claude mcp add ambit -- npx -y ambit-cli mcp` on a machine with Node and nothing else, or `brew install ambit` on macOS. The publish workflow exists and waits on a token. Until then an agent that decides Ambit would help cannot act on the decision without a checkout, which is the one remaining reason this loop stays a thing you set up instead of a thing you install.
