...
4. **No egress you did not type** — the graph is a local SQLite file and there is no telemetry. Four commands open a socket at all: `ambit notify` and `ambit notify-approvals`, each of which refuses to send without the topic argument you type; `ambit dispatch`, which refuses without a webhook URL from `--to` or `AMBIT_APPROVAL_WEBHOOK` and sends one proposal's summary or signed artifact and never a command (`propose --dispatch` and `approve --dispatch` are the same push); and `ambit incidents`, which probes the hosts that same manifest names and uploads nothing. A webhook is one-way: nothing that arrives on it can mint an approval, which only `ambit approve` on a machine holding the key can do. `runCommand` in `src/engine/cli.ts` is declared `async` for those alone; every other command finishes before the call returns.
