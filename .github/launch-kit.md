...
**Social preview.** Upload [`docs/assets/social-preview.png`](../docs/assets/social-preview.png) in **Settings → General → Social preview**. GitHub does not read it from the repository, so the upload has to be repeated whenever the card changes. The asset is 1280×640 and is regenerated by `npm run assets:generate`.
