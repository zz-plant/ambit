...
const MANIFEST_DEFAULT = join(process.env.HOME || '/', '.config', 'opencode', 'infrastructure.json');
...
async function incidents(db: Db) {
  const m = manifest();
  if (!m) {
    return { note: `No infrastructure manifest at ${process.env.INFRA_MANIFEST || MANIFEST_DEFAULT}. Nothing to watch.` };
  }
  ...
  for (const spec of services) {
    ...
    addEvent(db, run.run, { kind: 'detected', actor: 'monitoring', capabilityId: id, detail: `${spec.url} unreachable` });

    // The recovery decision, resolved exactly as apply would resolve it.
    const decision = canExecute(db, { actor: undefined, capability: RECOVERY_CAPABILITY, action: ACTION, target: id });
    addEvent(db, run.run, {
      kind: 'authority',
      actor: 'ambit',
      capabilityId: id,
