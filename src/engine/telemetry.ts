...
function beginRun(db: Migratable, input: BeginRunInput = {}) {
  ...
  return { run: id, started_at: (db.prepare("SELECT started_at FROM work_runs WHERE id = ?").get(id) as any)?.started_at };
}
...
function endRun(db: Migratable, runId: string, outcome: string, outcomeValueCents?: number) {
  ...
  db.prepare("UPDATE work_runs SET ended_at = datetime('now'), outcome = ?, outcome_value_cents = ? WHERE id = ?")
    .run(outcome, outcomeValueCents ?? null, runId);
  return { run: runId, outcome, ended_at: (db.prepare("SELECT ended_at FROM work_runs WHERE id = ?").get(runId) as any)?.ended_at };
}
...
function addEvent(db: Migratable, runId: string, event: EventInput) {
  ...
  ).run(runId, event.kind, event.actor || null, event.capabilityId || null, event.action || null, event.detail || null);
  return { run: runId, kind: event.kind, at: (db.prepare("SELECT at FROM work_events WHERE id = last_insert_rowid()").get() as any)?.at };
}
...
function recordIntervention(db: Migratable, runId: string | null, actorId: string, input: InterventionInput) {
  const ended = input.endedAt;
  const active = input.activeSeconds ?? (input.startedAt ? durationSeconds(input.startedAt, ended) : undefined);
  ...
  db.prepare(
    ...
    runId, actorId, input.kind,
    input.startedAt || new Date().toISOString().slice(0, 19).replace('T', ' '),
    ended || null,
    active ?? null, waiting ?? null,
    input.capabilityId || null, input.action || null, input.outcome || null
  );
  return { actor: actorId, kind: input.kind, active_seconds: active };
}

function recordResource(db: Migratable, runId: string | null, resourceId: string, kind: string, input: { quantity?: number; unit?: string; costCents?: number } = {}) {
  db.prepare(
...
function workReport(db: Migratable, limit = 20): any {
  ...
  if (runs.length === 0) return { note: 'No runs recorded. Begin one with a runtime adapter, or ambit record work.' };

  const nameOf = new Map(db.prepare("SELECT id, name FROM capabilities").all().map((c: any) => [c.id, c.name]));

  return runs.map((r) => {
    const elapsed = durationSeconds(r.started_at, r.ended_at);
    const events = (db.prepare("SELECT COUNT(*) n FROM work_events WHERE run_id = ?").get(r.id) as any).n;
    const uses = db.prepare("SELECT capability_id, SUM(duration_seconds) total FROM capability_use WHERE run_id = ? GROUP BY capability_id").all(r.id) as any[];
    ...
    const outcome = db.prepare("SELECT * FROM outcomes WHERE run_id = ? ORDER BY recorded_at DESC LIMIT 1").get(r.id);
    return {
      run: r.id,
      ...
      events,
      capabilities: uses.map((u) => ({ capability: nameOf.get(u.capability_id) || u.capability_id, duration_seconds: u.total })),
      interventions: interventions.map((i) => ({ kind: i.kind, times: i.n, active_seconds: i.active })),
...
function usageReport(db: Migratable, days = 30): any {
  ...
  for (const r of db.prepare("SELECT capability_id, COUNT(*) n FROM human_intervention WHERE started_at >= datetime('now', ?) GROUP BY capability_id").all(`-${days} days`) as any[]) {
    byIntervention.set(r.capability_id, r.n);
  }
  for (const r of db.prepare("SELECT run_id, COUNT(*) n FROM work_events WHERE at >= datetime('now', ?) GROUP BY run_id").all(`-${days} days`) as any[]) {
