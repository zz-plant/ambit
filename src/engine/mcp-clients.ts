...
function readCodexToml(raw: string): Record<string, unknown> | null {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (table) { current = servers[table[1]] = {}; continue; }
...
export function discoverMcpClients(home = process.env.HOME || "/"): McpClientSeed[] {
  ...
  for (const client of CLIENTS) {
    ...
    try { servers = client.read(readFileSync(path, "utf8")); } catch { continue; }
    if (!servers) continue;

    const mcp: Record<string, unknown> = {};
    for (const [name, server] of Object.entries<any>(servers)) {
      const remote = Boolean(server?.url || server?.serverUrl || server?.httpUrl || server?.type === "http" || server?.type === "sse");
