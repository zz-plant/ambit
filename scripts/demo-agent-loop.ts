...
function cli(...args: string[]): string[] {
  const r = spawnSync("node", ["--experimental-sqlite", join(ROOT, "src", "engine", "engine.ts"), ...args],
...
async function mcp(calls: { name: string; args: any }[]): Promise<string[][]> {
  ...
  send({ jsonrpc: "2.0", id: 0, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "demo-agent", version: "0" } } });
  calls.forEach((c, i) => send({ jsonrpc: "2.0", id: i + 1, method: "tools/call", params: { name: c.name, arguments: c.args } }));
  await done;
  proc.kill();
...
const [blockedOut, goalOut] = await mcp([
  { name: "ambit_blocked", args: { capId: "local-embeddings", classification: "infrastructure", note: "semantic search over the team wiki keeps failing" } },
...
if (!applyOut.some(l => l.includes("applied: true"))) throw new Error("apply did not succeed:\n" + applyOut.join("\n"));
...
const CMD = "#7dd3fc", DIM = "#64748b", OK = "#34d399", WARN = "#fbbf24", TXT = "#cbd5e1", HI = "#e2e8f0";
const take = (lines: string[], n: number) => lines.slice(0, n).map(l => ({ t: l.length > 88 ? l.slice(0, 85) + "…" : l, c: TXT }));

// JSON from MCP responses is dense; pull the lines that carry the story.
const pick = (lines: string[], keys: string[], n = 10): Line[] =>
  lines.filter(l => keys.some(k => l.includes(k))).slice(0, n)
    .map(l => ({ t: l.trim().replace(/[",]/g, "").slice(0, 86), c: TXT }));

const frames: Frame[] = [
  {
    title: "agent session — ambit over MCP", who: "agent", hold: 3,
    lines: [
  ...
  {
    title: "agent session — ambit over MCP", who: "agent", hold: 4,
    lines: [
  ...
  },
  {
    ...
    lines: [
      ...
      ...pick(proposeOut, ["proposal", "status", "applicable", "chosen", "privacy", "recurring"], 7),
      { t: "", c: TXT },
      { t: "  A draft, not an action. The agent can propose; only a person applies.", c: DIM },
    ],
  },
  {
    title: "human terminal", who: "human", hold: 3,
    lines: [
  ...
  {
    title: "human terminal", who: "human", hold: 4,
    lines: [
  ...
  {
    title: "human terminal", who: "human", hold: 4.5,
    lines: [
      ...
      { t: "  its prerequisites were finally met. That fact was written in no config file.", c: OK },
    ],
  },
];
...
execSync(`ffmpeg -y -loglevel error -framerate ${FPS} -i "${join(framesDir, "frame-%03d.png")}" -vf "split[s0][s1];[s0]palettegen=max_colors=128[p];[s1][p]paletteuse=dither=bayer" "${OUT_GIF}"`);
