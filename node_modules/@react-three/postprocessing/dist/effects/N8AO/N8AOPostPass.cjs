...
const FullScreenTriangle = require("./FullScreenTriangle.cjs");
const EffectShader = require("./EffectShader.cjs");
const EffectCompositer = require("./EffectCompositer.cjs");
...
const bluenoiseBits = buffer.Buffer.from(BlueNoise, "base64");
...
class N8AOPostPass extends postprocessing.Pass {
  /**
   *
  ...
  constructor(scene, camera, width = 512, height = 512) {
    ...
    this.configuration = new Proxy(
      {
        ...
        denoiseIterations: 2,
        renderMode: 0,
        color: new THREE__namespace.Color(0, 0, 0),
    ...
    this.configureHalfResTargets();
    this.copyQuad = new FullScreenTriangle.FullScreenTriangle(
      new THREE__namespace.ShaderMaterial({
        uniforms: {
          tDiffuse: {
            value: null
          }
    ...
    this.writeTargetInternal = new THREE__namespace.WebGLRenderTarget(this.width, this.height, {
      minFilter: THREE__namespace.LinearFilter,
      magFilter: THREE__namespace.LinearFilter,
      depthBuffer: false
    });
    this.readTargetInternal = new THREE__namespace.WebGLRenderTarget(this.width, this.height, {
      minFilter: THREE__namespace.LinearFilter,
      magFilter: THREE__namespace.LinearFilter,
      depthBuffer: false
    });
    this.outputTargetInternal = new THREE__namespace.WebGLRenderTarget(this.width, this.height, {
      minFilter: THREE__namespace.LinearFilter,
      magFilter: THREE__namespace.LinearFilter,
      depthBuffer: false
    });
    this.bluenoise = //bluenoise;
    new THREE__namespace.DataTexture(bluenoiseBits, 128, 128);
    this.bluenoise.colorSpace = THREE__namespace.NoColorSpace;
    this.bluenoise.wrapS = THREE__namespace.RepeatWrapping;
    this.bluenoise.wrapT = THREE__namespace.RepeatWrapping;
    this.bluenoise.minFilter = THREE__namespace.NearestFilter;
    this.bluenoise.magFilter = THREE__namespace.NearestFilter;
    this.bluenoise.needsUpdate = true;
    this.lastTime = 0;
    this.needsDepthTexture = true;
    this.needsSwap = true;
    this._r = new THREE__namespace.Vector2();
    this._c = new THREE__namespace.Color();
  }
  configureHalfResTargets() {
    if (this.configuration.halfRes) {
      ...
      this.depthDownsampleTarget.texture[0].magFilter = THREE__namespace.NearestFilter;
      this.depthDownsampleTarget.texture[0].depthBuffer = false;
      this.depthDownsampleTarget.texture[1].format = THREE__namespace.RGBAFormat;
      this.depthDownsampleTarget.texture[1].type = THREE__namespace.HalfFloatType;
      this.depthDownsampleTarget.texture[1].minFilter = THREE__namespace.NearestFilter;
      this.depthDownsampleTarget.texture[1].magFilter = THREE__namespace.NearestFilter;
      this.depthDownsampleTarget.texture[1].depthBuffer = false;
      this.depthDownsampleQuad = new FullScreenTriangle.FullScreenTriangle(new THREE__namespace.ShaderMaterial(DepthDownSample.DepthDownSample));
    } else {
  ...
  configureAOPass(logarithmicDepthBuffer = false) {
    ...
    const e = { ...EffectShader.EffectShader };
    ...
    if (this.effectShaderQuad) {
      ...
      this.effectShaderQuad.material = new THREE__namespace.ShaderMaterial(e);
    } else {
      this.effectShaderQuad = new FullScreenTriangle.FullScreenTriangle(new THREE__namespace.ShaderMaterial(e));
    }
  }
  configureDenoisePass(logarithmicDepthBuffer = false) {
    ...
    const p = { ...PoissionBlur.PoissionBlur };
    ...
    if (this.poissonBlurQuad) {
      ...
      this.poissonBlurQuad.material = new THREE__namespace.ShaderMaterial(p);
    } else {
      this.poissonBlurQuad = new FullScreenTriangle.FullScreenTriangle(new THREE__namespace.ShaderMaterial(p));
    }
  }
  configureEffectCompositer(logarithmicDepthBuffer = false) {
    const e = { ...EffectCompositer.EffectCompositer };
    if (logarithmicDepthBuffer) {
      e.fragmentShader = "#define LOGDEPTH\n" + e.fragmentShader;
    }
    ...
    if (this.effectCompositerQuad) {
      ...
      this.effectCompositerQuad.material = new THREE__namespace.ShaderMaterial(e);
    } else {
      this.effectCompositerQuad = new FullScreenTriangle.FullScreenTriangle(new THREE__namespace.ShaderMaterial(e));
    }
  }
  /**
   *
  ...
  generateDenoiseSamples(numSamples, numRings) {
    ...
    for (let i = 0; i < numSamples; i++) {
      samples.push(new THREE__namespace.Vector2(Math.cos(angle), Math.sin(angle)).multiplyScalar(Math.pow(radius, 0.75)));
      radius += radiusStep;
      angle += angleStep;
    }
    return samples;
  ...
  render(renderer, inputBuffer, outputBuffer) {
    ...
    this.effectShaderQuad.material.uniforms["cameraPos"].value = this.camera.getWorldPosition(new THREE__namespace.Vector3());
    ...
    for (let i = 0; i < this.configuration.denoiseIterations; i++) {
      ...
      this.poissonBlurQuad.material.uniforms["cameraPos"].value = this.camera.getWorldPosition(new THREE__namespace.Vector3());
    ...
    this.effectCompositerQuad.material.uniforms["cameraPos"].value = this.camera.getWorldPosition(new THREE__namespace.Vector3());
...
exports.N8AOPostPass = N8AOPostPass;
//# sourceMappingURL=N8AOPostPass.cjs.map
